...
// saga
function saga() {
  const target = path.join(cwd, './src/sagas')
  const questions = [{
    type: 'input',
    name: 'name',
    message: 'saga (path with file name): ',
  }]
  inquirer.prompt(questions).then(function (answers) {
    try {
      fs.copySync(path.join(templateDir, `./saga/saga.js`), path.join(target, answers.name), { overwrite: true });
      console.log(chalk.green(`Create saga success：${path.join(target, answers.name)}`))
    } catch (e) {
      console.log(chalk.red('Create saga Failed!'))
...
// reducer
function reducer() {
  const target = path.join(cwd, './src/reducers')
  const questions = [{
    type: 'input',
    name: 'name',
  ...
  inquirer.prompt(questions).then(function (answers) {
    try {
      fs.copySync(path.join(templateDir, `./reducer/reducer.js`), path.join(target, answers.name), { overwrite: true });
      console.log(chalk.green(`Create reducer success：${path.join(target, answers.name)}`))
    } catch (e) {
